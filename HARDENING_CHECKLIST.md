...
Tracks whop-product-lab issue #14. **Audit only — no fixes applied this round.** Per this repo's own `README.md` ("[STABLE VERSION - DO NOT MODIFY CORE LOGIC]"), every finding below is documented for a human/follow-up round; nothing under `src/` was touched. See `docs/specs/14-local-image-raw-family-hardening.md` for the canonical-base decision this checklist feeds into, and `NAMING.md` for the naming proposal.
